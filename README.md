...
`/decompose` 顶层返回 `themes`，每个名称限制为 2—6 个字符的精炼管理主题；文献先按相关性、决策价值、直接性、公司适用性和时效性整理，再用于生成主题，不会把“最高分文献”当作主题返回。默认配置已加入长鑫存储的组织背景，并按新技术、市场、政策、竞争对象和经营问题选择不同的领导关注重点，调用方仍可通过 `organization_context` 补充或覆盖。
